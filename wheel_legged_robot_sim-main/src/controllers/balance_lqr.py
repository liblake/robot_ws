...
# 左右腿电机 roll 差动控制的方向符号：
# 同一个虚拟 roll 力矩分配到左右腿时，需要按照模型约定取相反方向。
LEG_ROLL_DIFF_SIGNS: dict[str, float] = {
    MODEL_SEMANTICS.leg_motor_joints[0]: -1.0,  # 左腿电机使用负号，保证差动方向正确
    MODEL_SEMANTICS.leg_motor_joints[1]: 1.0,  # 右腿电机使用正号，与左腿形成反向 roll 力矩
}
