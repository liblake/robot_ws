...
LEG_MOTOR_JOINTS = (
    "link_002_joint",  # 右髋（用户确认：002 = 右腿髋）
    "link_003_joint",  # 右膝（用户确认：003 = 右腿膝）
    "link_005_joint",  # 左髋（用户确认：005 = 左腿髋）
    "link_006_joint",  # 左膝（用户确认：006 = 左腿膝）
)
LEG_LABELS = ("右髋", "右膝", "左髋", "左膝")
...
def main():
    ...
    from src.mjcf_builder import _urdf_to_mjcf, _configure_geom_collisions, _apply_link_materials
    ...
    # 串行腿机器人没有四连杆闭链，不需要 equality 约束。
    # （原开源四连杆模型在这里调用 _ensure_equality_constraints(root)。）
    ...
    with mujoco.viewer.launch_passive(model, data) as viewer:
        ...
        for joint_name, label in zip(LEG_MOTOR_JOINTS, LEG_LABELS):
            print(f"  act_{joint_name}  ->  {label}")
