...
def _urdf_to_mjcf(urdf_path: Path, output_dir: Path) -> Path:
    ...
    # 注意：有的 URDF（如 Two_wheeled_legged_robot）在 <mujoco><compiler>
    # 里用 meshdir="meshes" + 裸文件名；此时解析基准目录必须是
    # <urdf所在目录>/<meshdir>，不能只按 URDF 所在目录拼接。
    mesh_dir = urdf_path.parent
    compiler_el = urdf_root.find("./mujoco/compiler")
    meshdir = compiler_el.get("meshdir") if compiler_el is not None else None
    if meshdir:
        mesh_dir = (mesh_dir / meshdir).resolve()
    ...
    for mesh_el in urdf_root.iter("mesh"):
        ...
        if candidate.is_file():
            mesh_el.set("filename", str(candidate))
    ...
    # 归一化 root freejoint 名字：URDF 里的浮基关节（如 base_joint）转成
    # MJCF 后名字不是 "root"，而 state.py / controller 都按 "root" 查找。
    mjcf_tree = ET.parse(mjcf_path)
    mjcf_root = mjcf_tree.getroot()
    base_el = mjcf_root.find(".//body[@name='base_link']")
    if base_el is not None:
        for child in base_el:
            if child.tag in {"freejoint", "joint"} and child.get("type", "") == "free":
                child.set("name", "root")
...
def _add_collision_proxies(root: ET.Element) -> None:
    ...
    # 串联机器人：机身盒体 + 每条腿的大腿/小腿胶囊 + 两个轮子球体。
    # 胶囊 fromto 使用各连杆本地坐标系：从关节原点指向下一关节/轮轴原点
    # （数值取自 robot.urdf 里的 joint origin，与 mjcf_builder 无关）。
    # 轮子球体放在轮 body 原点（轮轴位置），半径 = WHEEL_RADIUS。
    _PROXY_DEFS: list[dict[str, str]] = [
        # base_link: 机身盒体（保守小于网格，站姿时不会碰到地面）
        {
            "body": "base_link",
            "type": "box",
            "size": "0.18 0.12 0.11",
        ...
        # 右腿大腿 link_002：髋(0,0,0) → 膝(0.072 -0.2935 -0.0622)
        {
            "body": "link_002",
            "type": "capsule",
            "size": "0.022",
            "fromto": "0 0 0 0.072 -0.2935 -0.0622",
        },
        # 右腿小腿 link_003：膝(0,0,0) → 轮轴(-0.022 0.2821 -0.1955)
        {
            "body": "link_003",
            "type": "capsule",
            "size": "0.022",
            "fromto": "0 0 0 -0.022 0.2821 -0.1955",
        },
        # 左腿大腿 link_005：髋(0,0,0) → 膝(-0.0667 -0.2935 -0.0622)
        {
            "body": "link_005",
            "type": "capsule",
            "size": "0.022",
            "fromto": "0 0 0 -0.0667 -0.2935 -0.0622",
        },
        # 左腿小腿 link_006：膝(0,0,0) → 轮轴(-0.0405 0.2821 -0.1955)
        {
            "body": "link_006",
            "type": "capsule",
            "size": "0.022",
            "fromto": "0 0 0 -0.0405 0.2821 -0.1955",
        },
        # 轮子：圆柱代理（半径=WHEEL_RADIUS，半长≈轮宽一半），轴线沿本体 X（轮轴）。
        # 球体会向侧面滚动，造成慢速 roll 晃动；圆柱更接近真实轮毂直驱轮。
        {
            "body": "link_007",
            "type": "cylinder",
            "size": f"{WHEEL_RADIUS} 0.04",  # 半径 半长
            "quat": "0.7071068 0 -0.7071068 0",  # 几何 z 轴转到 body X（轮轴）
            "friction": "1.0 0.02 0.001",
        },
        {
            "body": "link_004",
            "type": "cylinder",
            "size": f"{WHEEL_RADIUS} 0.04",
...
def _replace_actuators(root: ET.Element) -> None:
    ...
    # URDF 的 <limit effort="10"/> 会被 MuJoCo 转成 joint 上的
    # actuatorfrcrange="-10 10"（关节级力限制）。它会把腿电机实际力矩
    # 死死限制在 ±10 N·m，而我们 17kg 的机器站立需要远超 10 N·m。
    # 执行器量程统一由下方 ctrlrange 管理，这里删掉关节级限制。
...
def _ensure_equality_constraints(root: ET.Element) -> None:
    # 串联机器人没有四连杆闭链（connect_sites 为空），直接跳过。
    if not MODEL_SEMANTICS.connect_sites:
        return
...
# 串联机器人直立平衡站立位形：
#   wheel_y_offset = -0.075（轮心在髋后方 75mm），平衡 pitch≈1.6°，机身 z=0.44 贴地。
#   右腿：髋 -0.079720、膝 -0.179729；左腿镜像：髋 +0.079720、膝 +0.179729。
# qpos 顺序：freejoint(7) + 右髋/右膝/右轮 + 左髋/左膝/左轮 + cmd_* (4)。
_STANDING_KEYFRAME_QPOS = (
    "0 0 0.44 "    # base x y z
    "1 0 0 0 "     # base quat（直立）
    "-0.079720 -0.179729 0 "   # 右腿：髋 002、膝 003、轮 004
    "0.079720 0.179729 0 "     # 左腿：髋 005、膝 006、轮 007
    "0 0 0 0"      # cmd_linear_x, cmd_angular_z, cmd_height, cmd_jump
)


def _ensure_standing_keyframe(root: ET.Element) -> None:
    """Add a standing keyframe for the serial two-wheeled-legged robot."""
